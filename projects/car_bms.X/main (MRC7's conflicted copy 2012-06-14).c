...
main(void)
{
    ...
    IF_ERR(readFlash(&fd,sizeof(fd)), REP_WARNING, "readFlash error");

    /* initialize devices and warn on error */
    IF_ERR(init_devices(dp, ltcp, adcp, dsp, canp, serp, led0p, led1p),
    ...
    for (ui = 0; ui < ARRAY_SIZE(DS18X20_ROMCODES); ui++) {
        IF_ERR(dsp->op->verify(dsp, DS18X20_ROMCODES[ui]),
            ...
            REPORT((REP_EMERGENCY,
                    "TEMP %d, ROMCODE %X %X %X %X %X %X %X MISSING",
                    ui,
    ...
    while (1) {
        ...
        /* send heartbeat */
        IF_ERR(canp->op->tx(canp, COMMON_CAN_TX_CHN, STANDARD_ID,
...
init_leds(struct led *led0p, struct led *led1p)
{
    ...
    if (!led1p)
        ...
        IF_ERR(led_new(led1p, NU32_LED1_PORT, NU32_LED1_PIN),
                REP_WARNING, "led_new");
        ...
        struct led *led0p, struct led *led1p)
{
    ...
    IF_ERR(init_leds(led0p, led1p), REP_WARNING, "init_leds");

    IF_ERR(init_serial(serp), REP_WARNING, "init_serial");

    IF_ERR(init_nokia(dp), REP_WARNING, "init_nokia");

    IF_ERR(init_can(canp), REP_WARNING, "init_can");
...
void
nu_trip(struct can *canp, enum tripCode code, int32_t module)
{
...
sendLastResetCode(struct can *self)
{
    ...
    IF_ERR(self->op->tx(self, COMMON_CAN_TX_CHN, STANDARD_ID,
