...
sendErrFrame(const struct can *self, void *data, size_t len)
{
    ...
    return self->op->tx(self, self->error_reporting_can_chn,
                        ...
                        self->error_reporting_can_ext_id,
                        0,
                        data,
                        len);
}
...
CANRx (const struct can *self, CAN_CHANNEL channel, unsigned int *id, void *dst)
{
    ...
    CANRxMessageBuffer *message = (CANRxMessageBuffer *)CANGetRxMessage(self->module, channel);
    if (message == NULL)
        return -ENODATA;
    ...
    unsigned long len = (unsigned long) message->msgEID.DLC;
