...
static s32
led_resetErrState(struct error_reporting_dev *erd)
{
    led_off(erd_to_led(erd));
    return 0;
}

static s32
led_report(struct error_reporting_dev *erd,
...
{
    led_on(erd_to_led(erd));
    return 0;
}

const struct vtbl_error_reporting_dev led_erd_ops = {
    .report         = &led_report,
