/* for ltc6803 */
#include "../include/Ltc6803.h"
#include "../include/NUSPI.h"

static const unsigned char crctab[] =
   {};
    
static unsigned long
crctablefast (const BYTE *p, size_t len);

static inline BYTE
calcCmdPEC (BYTE cmd) __attribute__((always_inline));

/*
 * Based on code found at http://zorc.breitbandkatze.de/crc.html (scroll down on page)
 *
 * From LTC6803 data sheet:
 * characteristic polynomial x^8 + x^2 + x + 1
 * initial PEC value of 01000001 (0x41)
 *
 * Therefore, CRC calculator settings are as follows:
 * CRC order (1..64)    : 8
 * CRC polynom (hex)    : 07
 * Initial value (hex)  : 41 (direct)
 * Final XOR value (hex): 00
 * DON'T reverse data bytes
 * DON'T reverse CRC result before Final XOR
 */
static unsigned long
crctablefast (const BYTE *p, size_t len)
{
    /* fast lookup table algorithm without augmented zero bytes, e.g. used in pkzip.
     * only usable with polynom orders of 8, 16, 24 or 32.
     */

    unsigned long crc = 0x41;

    while (len--)
        crc = (crc << 8) ^ crctab[ (crc & 0xff) ^ *p++];
                                                            /* 8 here is referring to the polynomial order */
    static const unsigned long crcmask = ((((unsigned long)1<<(8-1))-1)<<1)|1;

    crc&= crcmask;

    return crc;
}

/* calculate PEC (CRC) code for a byte array and return as a byte */
static inline BYTE
calcCmdPEC (BYTE cmd)
{
    return (BYTE)crctablefast(&cmd, 1);
}

inline BYTE
calcPEC (const BYTE *in, size_t len)
{
	return (BYTE)crctablefast(in, len);
}

void
BPSInit(SpiChannel chn, const BYTE cfgr0, const BYTE cfgr1, const BYTE cfgr2, const BYTE cfgr3, const BYTE cfgr4, const BYTE cfgr5)
{
	SPI_CS = 0;
    delay_us(1);

    BYTE cmd = WRCFG;
    SpiChnPutC(chn, cmd);
    SpiChnPutC(chn, calcCmdPEC(cmd));

    /*
       buff[1] = calcPEC(buff, 1);
       SpiChnPutS(chn, (unsigned int *)buff, 2); // Sends config code 0x01 and corresponding PEC code C7
    */

    delay_us(10);

    /* byte array for top device, which is currently the only device */
    BYTE buff[7];
    buff[0] = cfgr0;
    buff[1] = cfgr1;
    buff[2] = cfgr2;
    buff[3] = cfgr3;
    buff[4] = cfgr4;
    buff[5] = cfgr5;
    buff[6] = calcPEC(buff, 6);
    /* buff[6] = 0x01; */
    int i;
    for (i = 0; i < 6; ++i)
    	SpiChnPutC(chn, buff[i]);

    /* then you would do the same for the middle device followed by the bottom device here */
    /* [code goes here] */

    /* SpiChnPutS(chn, buff, 1); */ /* Sends config codes for cfgr0-5 and corresponding PEC codes */
    delay_us(20);
    SPI_CS = 1;
    delay_us(1);
}

void
BPSInits(SpiChannel chn, unsigned int numDevices, union BpsConfig *cfgs)
{
	SPI_CS = 0;
    delay_us(1);

    BYTE cmd = WRCFG;
    SpiChnPutC(chn, cmd);
    SpiChnPutC(chn, calcCmdPEC(cmd));

    delay_us(10);

    unsigned int i;
    for (i = 0; i < numDevices; ++i) {
        cfgs[i].pec = calcPEC(cfgs[i].data, sizeof(cfgs[0].data));
        unsigned int j;
        for (j = 0; j < sizeof(cfgs[0].byteArr); ++j)
            SpiChnPutC(chn, cfgs[i].byteArr[j]);
    }

    delay_us(20);
    SPI_CS = 1;
    delay_us(1);
}

void
ReadCfg(SpiChannel chn, unsigned int numDevices, union BpsConfig *cfgs)
{
    SPI_CS = 0;
    delay_us(1);
    BYTE cmd = RDCFG;   /* Read Config Register */
    SpiChnPutC(chn, cmd);
    SpiChnPutC(chn, calcCmdPEC(cmd));
    delay_us(15);

    unsigned int i;
    for (i = 0; i < numDevices; ++i) {
        unsigned int j;
        for (j = 0; j < sizeof(cfgs[0].byteArr); ++j) {
            SpiChnGetRov(chn, 1);
            SpiChnPutC(chn, 0x00);
            if (i == 0 && j == 0)
                SpiChnGetC(chn);
            cfgs[i].byteArr[j] = (BYTE)SpiChnGetC(chn);
            SpiChnGetRov(chn, 1);
        }
    }
    delay_us(20);
    SPI_CS = 1;
    delay_us(1);
}

void
ReadRawVoltages(SpiChannel chn, unsigned int numDevices, union RawVoltages *cvrs)
{
    SPI_CS = 0;
    delay_us(1);
    BYTE cmd = RDCV;
    SpiChnPutC(chn, cmd);
    SpiChnPutC(chn, calcCmdPEC(cmd));
    delay_us(15);

    unsigned int i;
    for (i = 0; i < numDevices; ++i) {
        unsigned int j;
        for (j = 0; j < sizeof(cvrs[0].byteArr); ++j) {
            SpiChnGetRov(chn, 1);
            SpiChnPutC(chn, 0x00);
            if (i == 0 && j == 0)
                SpiChnGetC(chn);
            cvrs[i].byteArr[j] = (BYTE)SpiChnGetC(chn);
            SpiChnGetRov(chn, 1);
        }
    }
    delay_us(20);
    SPI_CS = 1;
    delay_us(1);
}

void
ConvertVoltages(unsigned int numDevices, union RawVoltages *cvrs, float *cellVoltages)
{
    unsigned int voltagePairsPerDevice = sizeof(cvrs[0].voltagePair)/sizeof(RawCellVoltagePair);
    unsigned int cellsPerDevice = voltagePairsPerDevice*2;
    unsigned int i;
    for (i = 0; i < numDevices; ++i) {
        unsigned int j;
        for (j = 0; j < voltagePairsPerDevice; ++j) {
            RawCellVoltagePair currentVPair = cvrs[i].voltagePair[j];
            cellVoltages[i*cellsPerDevice+j*2]      = ((double)currentVPair.voltage1-512)*0.0015;
            cellVoltages[i*cellsPerDevice+j*2+1]    = ((double)currentVPair.voltage2-512)*0.0015;
        }
    }
}

void
SendCmd(SpiChannel chn, BYTE cmd)
{
    SPI_CS = 0;
    delay_us(1);
    SpiChnPutC(chn, cmd);
    SpiChnPutC(chn, calcCmdPEC(cmd));
    delay_ms(20);
    SPI_CS = 1;
    delay_us(1);
}

void
ReadDagn(SpiChannel chn, unsigned int numDevices, union Diagnostic *dagn)
{
    SPI_CS = 0;
    delay_us(1);
    BYTE cmd = DAGN;
    SpiChnPutC(chn, cmd);
    SpiChnPutC(chn, calcCmdPEC(cmd));
    delay_ms(20);
    SPI_CS = 1;
    delay_us(5);

    SPI_CS = 0;
    delay_us(1);
    BYTE cmd1 = RDDGNR;
    SpiChnPutC(chn, cmd1);
    SpiChnPutC(chn, calcCmdPEC(cmd1));
    delay_us(20);

    unsigned int i;
    for (i = 0; i < numDevices; ++i) {
        unsigned int j;
        for (j = 0; j < sizeof(dagn[0].byteArr); ++j) {
            SpiChnGetRov(chn, 1);
            SpiChnPutC(chn, 0x00);
            if (i == 0 && j == 0)
                SpiChnGetC(chn);
            dagn[i].byteArr[j] = (BYTE)SpiChnGetC(chn);
            SpiChnGetRov(chn, 1);
        }
    }
    delay_us(20);
    SPI_CS = 1;
    delay_us(1);
}

void
ConvertDagn(unsigned int numDevices, union Diagnostic *dagn, unsigned int *ref, unsigned int *muxfail, unsigned int *rev)
{
    unsigned int i;
    for (i = 0; i < numDevices; ++i) {
        int combinedDagn;
        memcpy(&combinedDagn, dagn[0].byteArr, sizeof(dagn[0].byteArr));
        ref[i] = (0xFFF) & combinedDagn;
        muxfail[i] = (0x01) & (combinedDagn >> 13);
        rev[i] = (0x03) & (combinedDagn >> 14);
    }
}

void
SetVoltageLimits(union BpsConfig *cfg, double vuv, double vov)
{
    vuv = (vuv/(16*.0015)) + 31;
    vov = (vov/(16*.0015)) + 32;
    cfg->vuv = (BYTE) vuv;
    cfg->vov = (BYTE) vov;
}

inline UINT32
ConvertToRef(double vref)
{
    return (UINT32) ((vref/.0015) + 512);
}

BOOL
ltc6803POSTPassed(union BpsConfig *cfgs)
{    
    	/* Configuration register self test */
	BOOL pecMatch = FALSE;
    do {
        static UINT8 failcount = 0;
        if (failcount > MAXCFGSMATCHFAIL)
            return FALSE;
        failcount++;
            
        BPSInits(BPS_SPI_CHANNEL, LTC6803_COUNT, cfgs);
    	union BpsConfig writtenCfgs[LTC6803_COUNT];
    	ReadCfg(BPS_SPI_CHANNEL, LTC6803_COUNT, writtenCfgs);
    	
    	UINT8 i;
    	for (i = 0; i < LTC6803_COUNT; ++i) {
        	if (cfgs[i].pec != writtenCfgs[i].pec) {
        	    pecMatch = FALSE;
        	    break;
        	} else {
        	    pecMatch = TRUE;
        	}
        }
	} while (pecMatch == FALSE);
//	PRINTDEBUG();
	
	/* ADC self test */
	BOOL adcTestMatch = FALSE;
    do {
        static UINT8 failcount = 0;
        if (failcount > MAXADCMATCHFAIL)
            return FALSE;
        failcount++;
        
    	SendCmd(BPS_SPI_CHANNEL, STCVAD_SELFTEST1);
    	
    	union RawVoltages adcTest[LTC6803_COUNT];
        ReadRawVoltages(BPS_SPI_CHANNEL, LTC6803_COUNT, adcTest);
        
        UINT8 i;
        for (i = 0; i < LTC6803_COUNT; ++i) {
            if (adcTest[i].pec != calcPEC(adcTest[i].data, sizeof(adcTest[i].data))) {
                adcTestMatch = FALSE;
                break;
            }
            
            UINT8 j;
            for (j = 0; j < sizeof(adcTest[0].data); ++j) {
    	        if (adcTest[i].data[j] != 0x55) {
    	            adcTestMatch = FALSE;
    	            break;
    	        } else {
    	            adcTestMatch = TRUE;
    	        }
        	}
        	
    	    if (adcTestMatch == FALSE)
    	        break;
       	}
    } while (adcTestMatch == FALSE);
//    PRINTDEBUG();
    
    /* Multiplexer and REF voltage self test */
    BOOL dagnMatch = FALSE;
    do {
        static UINT8 failcount = 0;
        if (failcount > MAXDAGNFAIL)
            return FALSE;
        failcount++;
        
        SendCmd(BPS_SPI_CHANNEL, DAGN);
        
        union Diagnostic dagn[LTC6803_COUNT];
        ReadDagn(BPS_SPI_CHANNEL, LTC6803_COUNT, dagn);
        
        UINT8 i;
        for (i = 0; i < LTC6803_COUNT; ++i) {
            if (dagn[i].pec != calcPEC(dagn[i].data, sizeof(dagn[i].data))) {
                dagnMatch = FALSE;
                break;
            }
            
            if (dagn[i].muxfail == 1 || dagn[i].ref < ConvertToRef(2.1) || dagn[i].ref > ConvertToRef(2.9)) {
                dagnMatch = FALSE;
                break;
            } else {
                dagnMatch = TRUE;
            }
        }    
    } while (dagnMatch == FALSE);
//    PRINTDEBUG();

    return TRUE;
    /* End of POST */
}