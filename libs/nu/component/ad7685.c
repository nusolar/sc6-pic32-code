...
nu_ad7685_setup(struct nu_ad7685 *self)
{
    if ((NU_AD7685_FOUR_WIRE & self->opt && NU_AD7685_NO_BUSY_INDICATOR & self->opt) ||
        ...
        return -EINVAL;
    nu_pin_set_digital_out(&(self->convert_pin));
	struct nu_spi_platform_setup_args args = {(SpiOpenFlags) SPI_OFLAGS};
    nu_spi_setup(&(self->spi), spi_bitrate, &args);
    return 0;
}
